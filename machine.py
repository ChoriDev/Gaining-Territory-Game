...
class MACHINE():
    ...
    def find_best_selection(self):
        ...
        # 독립선분 최소 case에서 convex hull 위의 선분이 아닌 것을 선택 
    ...
    def simulation_independent_line(self, i, simul_line_set):
        ...
        # 독립변수 case 조사하는 개수 제한
        if len(self.independent_lines_case) == 12:
            return

        # 독립변수 case 조사할 때, 선분 수가 가장 적은 case보다 simulation set이 많아지면 즉시 종료
    ...
    # simulation_line_set에 포함된 선분을 대상으로 상대방에게 삼각형을 만드는 경우를 제공하는 선분인지를 판단
    ...
    def min_max_start(self, simul_line_set):
        ...
        depth = 0
        # 무조건 machine의 turn이니까 max_move 먼저 호출
